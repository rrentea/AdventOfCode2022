...
fn parse_input(
    ...
    let (input, crates) = crates(input)?;
    let (input, _) = newline(input)?;
    let (input, _number) = many1(preceded(space1, digit1))(input)?;
...
pub fn process_part1(input: &str) -> String {
    let (_, (mut crates, moves)) = parse_input(input).unwrap();
